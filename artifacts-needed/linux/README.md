...
- Ensure a `/dev/video*` output device exists for the app-owned CamJongUn camera.
...
- Use `v4l2loopback-ctl` when available so the CamJongUn-owned device can be
  added without unloading/reloading the kernel module.
